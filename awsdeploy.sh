...
awsRegion="us-east-1"
awsProfile="default"
nameSpace="demo" #needs to match nginx/site.conf php upstream
deployTag=$(date +"%s")
...
docker build -t $ecsRepositoryUri:nginx ./nginx >>./deploy.log 2>&1
docker build -t $ecsRepositoryUri:php ./php >>./deploy.log 2>&1
...
docker push $ecsRepositoryUri:nginx >>./deploy.log 2>&1
docker push $ecsRepositoryUri:php >>./deploy.log 2>&1

# Create required IAM Roles
echo "5/27  Creating IAM Role $deployTag-ecsTaskExecution"
taskExecutionRoleARN=$(aws iam create-role --role-name $deployTag-ecsTaskExecution --assume-role-policy-document file://./taskExecutionRolePolicy.json --region $awsRegion --profile $awsProfile | jq -r .Role.Arn)
...
aws ecs register-task-definition --container-definitions '[{"name": "php","image": "'$ecsRepositoryUri':php","essential": true, "portMappings": [{"hostPort": 9000,"protocol": "tcp","containerPort": 9000}]}]' --family "php" --requires-compatibilities "FARGATE"  --network-mode "awsvpc" --cpu "256" --memory "512" --task-role "$taskExecutionRoleARN" --execution-role-arn "$taskExecutionRoleARN" --region $awsRegion --profile $awsProfile >>./deploy.log 2>&1
aws ecs register-task-definition --container-definitions '[{"name": "nginx","image": "'$ecsRepositoryUri':nginx","essential": true, "portMappings": [{"hostPort": 80,"protocol": "tcp","containerPort": 80}]}]' --family "nginx" --requires-compatibilities "FARGATE"  --network-mode "awsvpc" --cpu "256" --memory "512" --task-role "$taskExecutionRoleARN" --execution-role-arn "$taskExecutionRoleARN" --region $awsRegion --profile $awsProfile >>./deploy.log 2>&1
...
aws ecs create-service --cluster $clusterArn --service-name "php" --task-definition "php" --desired-count 1 --service-registries '[{"registryArn":"'$PHPregistryArn'"}]' --launch-type "FARGATE" --network-configuration "awsvpcConfiguration={subnets=[$subnetId1,$subnetId2],securityGroups=[$securityGroupId],assignPublicIp=ENABLED}" --region $awsRegion --profile $awsProfile >>./deploy.log 2>&1
...
ALBsecurityGroupId=$(aws ec2 create-security-group --description "Security Group for Fargate ALB" --group-name "demo-albsg" --vpc-id $vpcId --region $awsRegion --profile $awsProfile | jq -r .GroupId)
...
aws ecs create-service --cluster $clusterArn --service-name "nginx" --task-definition "nginx" --desired-count 1 --load-balancers "targetGroupArn=$nginxTargetGroupARN,containerName=nginx,containerPort=80" --launch-type "FARGATE" --network-configuration "awsvpcConfiguration={subnets=[$subnetId1,$subnetId2],securityGroups=[$securityGroupId],assignPublicIp=ENABLED}" --region $awsRegion --profile $awsProfile >>./deploy.log 2>&1
